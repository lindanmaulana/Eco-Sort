...
- [x] **Sistem Nyawa Dinamis & Penalti (New):** Implementasi sistem 300 Max HP dengan visualisasi modular 5 hati (`AppHeartsUI`) yang terikat dengan nilai `penaltyPoint` (B3 memotong lebih besar).
- [x] **Teks Indikator HP Terperinci (New):** Sinkronisasi teks angka (Contoh: `298 / 300`) menggunakan TextMeshPro untuk melacak pengurangan HP kecil secara presisi.
- [x] **Sistem Win & Lose State Terpusat (New):** - Logika Game Over instan saat darah habis (`0`).
...
- [ ] **Fungsionalitas Tombol Panel Akhir:** Menghubungkan tombol *Restart* dan *Main Menu* pada Panel Game Over dan Game Win menggunakan fungsi `SceneManager.LoadScene`.
